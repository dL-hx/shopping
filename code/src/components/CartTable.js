...
class CartTable extends Component {
  ...
  // 更改购物车中的商品数量
  changeNum(id, e) {
    const { addCartItemNumServer } = this.props;
    addCartItemNumServer({ cid: id, count: Number( e.target.value) });
  }

  render() {
    const { cartList, total , delCartItemServer } = this.props;
                ...
                <div className="cart-row" key={item.id}>
                  ...
                  <div className="cart-quantity cart-column">
                    <input
                      ...
                      type="number"
                      value={item.count}
                      onChange={(e) => this.changeNum(item.id, e)}
                    />
                    <button className="btn btn-danger" type="button" onClick={()=>delCartItemServer({ cid: item.id })}>
                      删除
                    </button>
                  </div>
                </div>
